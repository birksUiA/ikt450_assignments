...
class static_name:
    ...
    def _set_up(model_name="unnamed_model"):
        ...
        static_name.output_dir = os.path.join(
            "output", static_name.model_name + "-" + static_name.time_string
        )
...
def plot_images_from_set(
    ...
    number_of_images: int = 9,
    image_grid: tuple[int, int] = (3, 3),
...
def plot_multiple_lines(
    ...
    for i, x in enumerate(xs):
        plt.plot(x, label=legneds[i], linewidth=line_width)
        ...
        if sub_dir is not None:
            ...
            plt.savefig(
                static_name.get_timed_file_path(
                    file_name=f"Confusion_matrix_{epoch}", sub_dir=sub_dir
                ),
                dpi=static_name.dpi,
            )
        else:
